...
public class Main {
	...
	public static Cellobj[][] sortd(Cellobj[][] Array, String input) {
		...
		Cellobj[] tot = new Cellobj[(columns) * rows];
		int strcount = 0;
		...
		for (int idx = 0; idx <= columns - 1; idx++) {// keep eye on this,
			...
			// need to start at
			// 1
			Cellobj[] arr = new Cellobj[rows];

			int x = 0;

			for (int i = c1.v; i <= c2.v; i++) {
				arr[x] = Array[c1.h + idx][i];
				x++;
				if (Array[c1.h + idx][i].DataType.equals("string")) {
					strcount++;
				}
		...
		int x = 0;
		Cellobj constant = new Cellobj(
				-10000000000000000003424322432143215.2543543276584);

		for (int idx = 0; idx < tot.length; idx++) {
			if (tot[idx].DataType.equals("string")) {
				strs[x] = tot[idx].internalstr;
				x++;
				tot[idx] = constant;
			}
		}

		double[] total = new double[tot.length];

		for (int idx = 0; idx < tot.length; idx++) {
			total[idx] = tot[idx].internaldouble;
		}
		...
		for (int idx = columns - 1; idx >= 0; idx++) {
			for (int i = c1.v; i <= c2.v; i++) {
				System.out.println(total[counter]);
				if (total[counter] == constant.internaldouble) {
					Array[c1.h + idx][i] = new Cellobj(strs[other]);
	...
	public static double Average(Cellobj[][] Array, String input) {
		...
		if (c1.h == c2.h) {
			...
			for (int i = c1.v; i <= c2.v; i++) {
				arr[x] = Array[c1.h][i].internaldouble;
				x++;
			}
			...
			int length = (c2.h - c1.h) + 1;

			double[] arr = new double[length];

			int x = 0;

			for (int i = c1.h; i <= c2.h; i++) {
				arr[x] = Array[i][c1.v].internaldouble;
				x++;
			}
